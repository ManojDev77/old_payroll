...
String empDOJ = '';
int newcircularcount = 0;
List<NoticeModal> mainNoticeList = [];
int notreadcount = 0;
int leaveleftapprovecnt = 0;
int widgetcnt = 0;

class TaskandAttendance extends StatelessWidget {
  const TaskandAttendance({super.key});
...
class BuildLandingPage extends StatelessWidget {
  const BuildLandingPage({super.key});
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> noticelog2() async {
    String query =
        '${globals.applictionRootUrl}API/NoticeDetailsEmpLog?DBName=${globals.databaseName}&UserId=${globals.userId}';
  ...
  showDialogIfFirstLoaded() async {
    if (!globals.leaveisWithoutLocationEnabled) {
  ...
  Future _getSettingsDetailsleave() async {
    ...
    if (response.statusCode == 200) {
      ...
      var photocap = jobject["SettingData"]["LoginPhotoCapture"];
  ...
  Future _getFilterDashboardData() async {
    ...
    final http.Response response = await http.post(
      Uri.parse(
          '${globals.applictionRootUrl}API/AdminOnSelectionDashboardData?DBName=${globals.databaseName}&UserId=${globals.userId}&MonthId=$monthValue'),
      headers: <String, String>{
        'Content-Type': 'application/x-www-form-urlencoded',
      },
    );
